...
class SupabaseCollectionRef {
    ...
    async get() {
        // Fix 500 Statement Timeout for Products by excluding heavy columns
        let selectStr = '*';
        if (this.name === 'products') {
            selectStr = 'id, productId, name, brand, category, partModel, partType, price, specs, emoji, isOutOfStock, variations, badge, tags, created_at, lastUpdatedBy, updatedAt';
        }
