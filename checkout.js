                ...
                (async () => {
                    try {
                        ...
                        if (user && user.email) {
                            ...
                            const redemptionsSnap = await (await supabase.from('vouchers_redemptions').where('email', '==', email).select('*')).data;
                            if (!redemptionsSnap.empty) {
                                ...
                                for (let i = 0; i < claimedCodes.length; i += chunkSize) {
                                    ...
                                    const vouchersSnap = await (await supabase.from('vouchers').where('code', 'in', chunk).select('*')).data;
                            ...
                            const ordersSnap = await (await supabase.from('orders').where('customerEmail', '==', email).select('*')).data;
